...
#    查矩阵 A 得到工序 1 ~ 工序 末位 的机台数上限 UpperBound[p];
...
#   工序 1 ~ 工序 11 -> 通过 PROCESS_TO_MATRIX_COL 映射到矩阵 A 的列;
...
def compute_actual_machines(line_count, total_lines=None, ratio=None):
    ...
    3. 对其他工序 p:
       - a_total_p = MATRIX_A[total_lines - 1][col(p)]   (日内总配额)
       - share = a_total_p × line_count / total_lines     (按 line_count 比例切分)
       - 若 share 为整数 → 返回 int (机台数刚好整数, 便于现场确定机台编号)
         否则 → 返回 float (按比例切到小数, 现场协调)

    特性 (与"独立计算"版本对比):
      - 同一天的所有订单 *同时* 确定 (共享 A(L_total) 配额);
      - sum(Actual[p] over all orders) = A(L_total)[p] (无拆单加成);
      - 表4 差异(M-A) 行应该恒为 0;
      - 已知黑豹拿 6, 则蜜蜂自动拿到 A(L_total) - 6, 不会超量。
    ...
    # 矩阵 A 的行索引: total_lines 条大线对应第 (total_lines - 1) 行
    matrix_row = MATRIX_A[total_lines - 1]
    ...
    # 其他工序: 按 line_count / total_lines 比例切分日内配额
    for p_idx in range(FINAL_PROCESS_IDX):
        a_total_p = matrix_row[PROCESS_TO_MATRIX_COL[p_idx]]
...
#   2. M 矩阵汇总 (当天 11 个工序的机台总数);  # zgy: SE 移除后为 11 个
#   3. A 矩阵理论 (根据当天总产线数查矩阵 A 得到的机台数);
#   4. 差异 (M - A), 正值 = 机台不够 (突破 A 上限), 负值 = 空余机台;
#   5. 备注: 把"机台不够"和"空余"的工序点出来。


def _build_a_reference_row(total_lines):
    """
    根据当天总产线数, 查矩阵 A 得到每个工序的理论机台数。

    返回长度 NUM_PROCESSES (=11) 的列表; total_lines 不在 [1, 18] 时返回全 None。
    (zgy: SE 工序已移除, 通过 PROCESS_TO_MATRIX_COL 跳过 MATRIX_A 的 SE 列)
    """
    if total_lines < 1 or total_lines > 18:
        return [None] * NUM_PROCESSES

    a_row = MATRIX_A[total_lines - 1]
    return [
        a_row[PROCESS_TO_MATRIX_COL[p_idx]]
        for p_idx in range(NUM_PROCESSES)
    ]
...
def _summarize_diff(m_sum, a_machines):
    """
    用一句简短文字描述当天工序的 M-A 差异:
    - 正值 (M > A) -> "机台不够"
    - 负值 (M < A) -> "空余"
    (zgy: SE 工序已移除; 整数差异不显示小数, 浮点差异保留 2 位小数)
    """
    if a_machines[0] is None:
        return ""

    short_p_names = [
        f"工序{i + 1}" for i in range(NUM_PROCESSES)
    ]

    deficits = []
    surplus = []

    # zgy: 用一个小阈值判断, 避免浮点数微小误差
    EPS = 1e-9

    def _fmt(diff_value):
        # zgy: 整数差异不带小数, 浮点差异带 2 位小数
...
def parse_date_machine_summary_view(
    ...
    每天一个分块, 包含以下行:
      1. 当天每个活跃订单一行 (按 A(L_total) 比例切分);
      2. M 矩阵汇总 (订单维度合并);
      3. A 矩阵理论 (按当天总产线数查矩阵 A);
      4. 差异(M-A): zgy: 按比例分配后理论上恒为 0;
      5. 空行分隔。

    DataFrame 列结构 (zgy: SE 工序已移除, 工序列从 12 减到 11):
      日期 / 类型/订单 / 占用产线 / 工序1-制绒 ... 工序11-丝网 / 备注

    备注列在"差异"行里指出哪些工序不够 / 哪些工序空余,
    其他类型行的备注列为空字符串。
    """
    if ratio is None:
        ratio = MACHINE_RATIO
    ...
    for display_date in display_dates:
        ...
        # =========================
        # 1. 第一遍: 收集当天所有活跃订单 + 算 total_lines
        # =========================
        # zgy: 必须先确定 total_lines, 才能让 compute_actual_machines 按比例切分
        active_orders = []
        total_lines = 0
        for j, order in enumerate(orders):
            line_count = solver.Value(l[j, t])
        ...
        # =========================
        # 2. 第二遍: 用 total_lines 同时确定每个订单的机台数
        # =========================
        # zgy: 现在所有订单"同时"确定 (共享 total_lines), 而非"逐个独立"
        day_orders = []
        # zgy: m_sum 用 int 初始化, 加法时类型按 actual 元素自动提升
        ...
        row_a = {
            "日期": date_str,
            "类型/订单": f"A矩阵理论(L={total_lines})",
            "占用产线": total_lines,
            "备注": "按当天总产线数从矩阵 A 查表",
        }
        for p_idx, col_name in enumerate(process_columns):
            row_a[col_name] = (
                a_machines[p_idx] if a_machines[p_idx] is not None else ""
            )
        rows.append(row_a)
        ...
        # 6. 差异 (M - A) 行
        # =========================
        # zgy: 按比例分配后, M_sum 应严格等于 A(L_total), 此行恒为 0,
        # 仅作为 sanity check 保留 (浮点累加可能有微小误差)。
        row_diff = {
            "日期": date_str,
            "类型/订单": "差异(M-A)",
            "占用产线": "",
    ...
    # 删除最后多余的空行
    while rows and all(
        v == "" or v is None for v in rows[-1].values()
    ):
        rows.pop()
